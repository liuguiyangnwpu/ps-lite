...
public:
  ...
  /**
   * add an customer to the system. threadsafe
   */
    void AddCustomer(Customer* customer);
  /**
   * remove a customer by given it's id. threasafe
   */
    void RemoveCustomer(Customer* customer);
  /**
   * get the customer by id, threadsafe
  ...
  /**
   * get the id of a node (group), threadsafe
   *
  ...
  /* return the key ranges of all server nodes */
    const std::vector<Range>& GetServerKeyRanges();
  /**
   * the template of a callback
   */
...
private:
    ...
    /** Holding a shared_ptr to prevent it from being destructed too early */
