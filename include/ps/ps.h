 ...
 * @file   ps.h
 *  The parameter server interface
 */
#ifndef PS_PS_H_
#define PS_PS_H_
/** basic setups in ps */
#include "ps/base.h"
/** communicating with a pair of (int, string). */
#include "ps/simple_app.h"
/** communcating with a list of key-value paris. */
#include "ps/kv_app.h"
namespace ps {
/** Returns the number of worker nodes */
inline int NumWorkers() { return Postoffice::Get()->num_workers(); }
/** Returns the number of server nodes */
inline int NumServers() { return Postoffice::Get()->num_servers(); }
/** Returns true if this node is a worker node */
inline bool IsWorker() { return Postoffice::Get()->is_worker(); }
/** Returns true if this node is a server node. */
inline bool IsServer() { return Postoffice::Get()->is_server(); }
/** Returns true if this node is a scheduler node. */
inline bool IsScheduler() { return Postoffice::Get()->is_scheduler(); }
/** Returns the rank of this node in its group
 *
...
/**
 * Register a callback to the system which is called after Finalize()
 *
