...
public:
  ...
  /**
   * add an incomming message
   * return true if msg has been added before or a ACK message
   */
...
private:
    ...
    uint64_t GetKey(const Message& msg) {
        ...
        uint8_t sender = msg.meta.sender == Node::kEmpty ? van_->my_node().id : msg.meta.sender;
